...
function getAidPackage(int packageId) returns AidPackage|error {
    AidPackage aidPackage = check dbClient->queryRow(`SELECT PACKAGEID, NAME, DESCRIPTION, STATUS, UNIX_TIMESTAMP(DATETIME) as 'dateTime', CREATEDBY as 'createdBy' FROM AID_PACKAGE
...
function getAidPackages(string? status) returns AidPackage[]|error {
    ...
    sql:ParameterizedQuery query = `SELECT PACKAGEID, NAME, DESCRIPTION, STATUS, UNIX_TIMESTAMP(DATETIME) as 'dateTime', CREATEDBY as 'createdBy' FROM AID_PACKAGE`;
    if (status is string) {
...
function addAidPackage(AidPackage aidPackage) returns int|error {
    int packageId = -1;
    int currentTime = getEpoch();
    sql:ExecutionResult result = check dbClient->execute(`INSERT INTO AID_PACKAGE(NAME, DESCRIPTION, STATUS, DATETIME, CREATEDBY)
                                        VALUES (${aidPackage.name}, ${aidPackage.description}, ${aidPackage.status}, FROM_UNIXTIME(${currentTime}), ${aidPackage?.createdBy});`);
    var lastInsertedID = result.lastInsertId;
    if lastInsertedID is int {
